...
packages <- c("tinytex", "haven", "tidyverse", "stargazer", "knitr", "glmnet", "pls", "mltools", "data.table", "readr", "corrplot")
...
The data set that we will be working with in this final project is called the Ames Iowa housing data set. This data set contains 2,909 observations across 81 variables, with each row representing a single house in Ames, Iowa. There are 1461 observations in the training data set, and 1448 observations in the testing data set. The 81 variables contain various measures and observations about the size, price, location, quality, condition, features, and value of each home and the plot of land it is on. This includes measures related to each home’s garage, roof, driveway, total area, bathrooms, bedrooms, and more.
...
Based on the plots above, there aren't too many immediate issues. The correlation between total basement area and first floor area is relatively high, but some robust models should be able to handle this while making use of the additional lot information.
